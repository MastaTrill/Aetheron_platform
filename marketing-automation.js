...
class AetheronMarketing {
    ...
    loadPostedHistory() {
        try {
            ...
            if (saved) {
                ...
                this.postedToday = new Set(
                    data.filter(timestamp => timestamp > cutoff)
                );
            }
        } catch (e) {
            this.postedToday = new Set();
    ...
    savePostedHistory() {
        try {
            localStorage.setItem('aetheron_posted', 
                JSON.stringify([...this.postedToday])
            );
        } catch (e) {
...
Stake here: ${this.links.staking}`
    ...
    showCopyModal(type, content) {
        ...
        modal.innerHTML = `
            ...
            <textarea style="width: 100%; height: 200px; margin-bottom: 10px;">${content}</textarea>
...
2. Send to MetaMask
3. Bridge to ${this.chain} via wallet.${this.chain.toLowerCase()}.technology

...
contract AetheronStaking is Ownable, ReentrancyGuard {
    ...
    function stake(uint256 _poolId, uint256 _amount) external nonReentrant {
        ...
        uint256 beforeBal = aetheronToken.balanceOf(address(this));
        aetheronToken.safeTransferFrom(msg.sender, address(this), _amount);
        uint256 afterBal = aetheronToken.balanceOf(address(this));
        uint256 actualReceived = afterBal - beforeBal;

        // Create stake record with actual received
        ...
        // Update totals
        pool.totalStaked += actualReceived;
        totalStaked += actualReceived;

        emit Staked(msg.sender, _poolId, actualReceived);
    }
    ...
    function claimRewards(uint256 _stakeId) external nonReentrant {
        ...
        // Track balance before
        uint256 beforeBal = aetheronToken.balanceOf(msg.sender);
        aetheronToken.safeTransfer(msg.sender, reward);
        uint256 afterBal = aetheronToken.balanceOf(msg.sender);
        uint256 netReceived = afterBal - beforeBal;

        emit RewardClaimed(msg.sender, _stakeId, reward, netReceived);
    }
    ...
    function unstake(uint256 _stakeId) external nonReentrant {
        ...
        if (_stakeId != lastIndex) {
            userStakes[msg.sender][_stakeId] = userStakes[msg.sender][
                lastIndex
            ];
        }
        ...
        emit Unstaked(msg.sender, _stakeId, amount, netReceived);
        if (grossReward > 0) {
            // Calculate net reward by difference
            netReward = netReceived > userStake.amount
                ? netReceived - userStake.amount
                : 0;
            emit RewardClaimed(msg.sender, _stakeId, grossReward, netReward);
        }
    }

...
describe("Aetheron Token", function () {
  ...
  describe("Deployment", function () {
    ...
    it("Should distribute tokens correctly", async function () {
      ...
      expect(ownerBalance).to.equal(TOTAL_SUPPLY.mul(50).div(100)); // 50%
      expect(teamBalance).to.equal(TOTAL_SUPPLY.mul(20).div(100)); // 20%
      expect(marketingBalance).to.equal(TOTAL_SUPPLY.mul(15).div(100)); // 15%
      expect(stakingBalance).to.equal(TOTAL_SUPPLY.mul(15).div(100)); // 15%
    });
  ...
  describe("Trading Control", function () {
    it("Should not allow trading before enabled", async function () {
      await aetheron.transfer(user1.address, ethers.utils.parseEther("1000"));
      
      await expect(
        aetheron.connect(user1).transfer(user2.address, ethers.utils.parseEther("100"))
  ...
  describe("Tax System", function () {
    beforeEach(async function () {
      await aetheron.enableTrading();
      await aetheron.transfer(user1.address, ethers.utils.parseEther("10000"));
    });
    
    it("Should apply tax on transfers", async function () {
      const transferAmount = ethers.utils.parseEther("1000");
      const expectedTax = transferAmount.mul(5).div(100); // 5% sell tax
      const expectedReceived = transferAmount.sub(expectedTax);
      ...
      expect(balanceAfter.sub(balanceBefore)).to.be.lt(transferAmount);
    });
  ...
  describe("Wallet Management", function () {
    ...
    it("Should not allow zero addresses for wallets", async function () {
      await expect(
        aetheron.updateWallets(ethers.constants.AddressZero, user2.address, stakingPool.address)

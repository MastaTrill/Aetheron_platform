...
async function main() {
  ...
  // const mintTx = await aetheron.mint(signer.address, ethers.parseUnits("1000", 18));
  // await mintTx.wait();
  // console.log("Minted 1000 tokens");

  // Transfer tokens
  console.log("Transferring tokens...");
  const transferTx = await aetheron.transfer("<RECIPIENT_ADDRESS>", ethers.parseUnits("10", 18));
  await transferTx.wait();
  console.log("Transferred 10 tokens");

  // Approve staking contract
  console.log("Approving staking contract...");
  const approveTx = await aetheron.approve(STAKING_ADDRESS, ethers.parseUnits("10", 18));
  ...
  const stakeTx = await staking.stake(ethers.parseUnits("10", 18));
  await stakeTx.wait();

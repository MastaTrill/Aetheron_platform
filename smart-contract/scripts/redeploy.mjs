...
async function main() {
  ...
  console.log(
    colors.bold +
      colors.cyan +
      '🚀 AETHERON PLATFORM - FRESH DEPLOYMENT' +
      colors.reset,
  );
  ...
  console.log(
    colors.green + '✅ Configuration validated successfully!\n' + colors.reset,
  );
  ...
  console.log(
    ...
    colors.cyan + deployer.address + colors.reset,
  );

  const balance = await ethers.provider.getBalance(deployer.address);
  console.log(
    '💰 Account balance:',
    colors.cyan + ethers.formatEther(balance) + ' POL' + colors.reset,
  );
  ...
  if (balance === 0n) {
    ...
    console.log(
      '   Please fund your wallet before deploying: ' + deployer.address,
    );
    process.exit(1);
  }
  ...
  console.log(
    '  Marketing Wallet:',
    colors.cyan + MARKETING_WALLET + colors.reset,
  );
  ...
  try {
    ...
    console.log(
      colors.bold +
        'Step 1/5: Deploying Aetheron Token Contract...' +
        colors.reset,
    );
    console.log('-'.repeat(60));
    ...
    console.log(
      colors.bold +
        'Step 2/5: Deploying Aetheron Staking Contract...' +
        colors.reset,
    );
    console.log('-'.repeat(60));
    ...
    // Step 3: Deploy AetxToken
    console.log('\n' + '-'.repeat(60));
    console.log(
      colors.bold + 'Step 3/6: Deploying AetxToken Contract...' + colors.reset,
    );
    console.log('-'.repeat(60));

    const AetxToken = await ethers.getContractFactory('AetxToken');
    const aetxToken = await AetxToken.deploy(TEAM_WALLET);

    await aetxToken.waitForDeployment();
    const aetxTokenAddress = await aetxToken.getAddress();
    console.log(
      colors.green +
        '✅ AetxToken deployed to: ' +
        aetxTokenAddress +
        colors.reset,
    );

    // Step 4: Update staking pool address in token contract
    console.log('\n' + '-'.repeat(60));
    console.log(
      colors.bold +
        'Step 4/6: Updating Token Contract Configuration...' +
        colors.reset,
    );
    console.log('-'.repeat(60));
    ...
    console.log(
      colors.green + '  ✅ Staking contract excluded from tax' + colors.reset,
    );

    // Step 5: Transfer and setup staking rewards
    console.log('\n' + '-'.repeat(60));
    console.log(
      colors.bold + 'Step 5/6: Setting Up Staking Rewards...' + colors.reset,
    );
    console.log('-'.repeat(60));
    ...
    console.log(colors.green + '  ✅ Approval complete' + colors.reset);
    ...
    console.log('\n' + '-'.repeat(60));
    console.log(
      colors.bold + 'Step 6/6: Verifying Deployment...' + colors.reset,
    );
    console.log('-'.repeat(60));
    ...
    console.log(
      '  ✅ Marketing balance:',
      ethers.formatEther(marketingBalance),
      'AETH',
    );
    console.log(
      '  ✅ Deployer balance:',
    ...
    console.log(
      '  ✅ Reward pool balance:',
      ethers.formatEther(rewardBalance),
      'AETH',
    );
    ...
    console.log('\n' + '='.repeat(60));
    console.log(
      colors.bold + colors.green + '🎉 DEPLOYMENT SUCCESSFUL!' + colors.reset,
    );
    console.log('='.repeat(60));
    ...
    console.log(
      '  AETH Token:        ',
      colors.cyan + aetheronAddress + colors.reset,
    );
    console.log(
      '  Staking Contract:  ',
      colors.cyan + stakingAddress + colors.reset,
    );
    ...
    console.log(
      '  ├─ Marketing:      ',
      ethers.formatEther(marketingBalance),
      'AETH → Marketing Wallet',
    );
    console.log(
      '  └─ Staking:        ',
      ethers.formatEther(stakingBalance),
      'AETH → Staking Contract',
    );
    ...
    console.log(
      '  Trading Enabled:   ',
      tradingEnabled ? '✅ YES' : '❌ NO (call enableTrading())',
    );
    ...
    console.log(
      '  2. ' +
        colors.yellow +
        'Update .env file with AETH_TOKEN_ADDRESS:' +
        colors.reset,
    );
    ...
    console.log(
      '  AETH Token: https://polygonscan.com/address/' + aetheronAddress,
    );
    console.log(
      '  Staking:    https://polygonscan.com/address/' + stakingAddress,
    );
    console.log(
      '  AETX Token: https://polygonscan.com/address/' + aetxTokenAddress,
    );
    ...
    const deploymentInfo = {
      ...
      contracts: {
        ...
        AetxToken: {
          address: aetxTokenAddress,
          totalSupply: ethers.formatEther(await aetxToken.totalSupply()),
        },
      },
      wallets: {
        team: TEAM_WALLET,
    ...
    if (error.message.includes('insufficient funds')) {
      ...
      console.log(
        '   Add more POL to your deployer wallet: ' + deployer.address,
      );

...
function validatePrivateKey(privateKey) {
  ...
  if (keyWithoutPrefix.length !== 64) {
    return {
      valid: false,
      error:
        `PRIVATE_KEY must be 64 hexadecimal characters (66 total with 0x)\n` +
  ...
  if (!/^[0-9a-fA-F]+$/.test(keyWithoutPrefix)) {
    return {
      valid: false,
      error:
        'PRIVATE_KEY must contain only hexadecimal characters (0-9, a-f, A-F)',
    };
  }
  ...
  if (allZeros || allOnes || sequential) {
    return {
      valid: false,
      error:
        'PRIVATE_KEY appears to be a placeholder/example value\n' +
...
function validateAddress(address, varName) {
  ...
  if (trimmedAddress !== address) {
    return {
      valid: false,
      error:
        `${varName} contains extra whitespace\n` +
  ...
  if (!trimmedAddress.startsWith('0x')) {
    return {
      valid: false,
      error:
        `${varName} must start with 0x prefix\n` +
  ...
  if (trimmedAddress.length !== 42) {
    return {
      valid: false,
      error:
        `${varName} must be 42 characters (0x + 40 hex characters)\n` +
  ...
  if (!/^0x[0-9a-fA-F]{40}$/.test(trimmedAddress)) {
    return {
      valid: false,
      error:
        `${varName} must contain only hexadecimal characters\n` +
  ...
  if (trimmedAddress === '0x0000000000000000000000000000000000000000') {
    return {
      valid: false,
      error:
        `${varName} is set to zero address (0x0...0)\n` +
  ...
  try {
    ...
    if (
      trimmedAddress !== checksumAddress &&
      trimmedAddress !== checksumAddress.toLowerCase()
    ) {
      return {
        valid: true,
        warning:
          `${varName} checksum is invalid but address format is correct\n` +
          `  Current:  ${trimmedAddress}\n` +
...
function validateRpcUrl(url, varName) {
  ...
  if (!trimmedUrl.startsWith('http://') && !trimmedUrl.startsWith('https://')) {
    return {
      valid: false,
      error:
        `${varName} must start with http:// or https://\n` +
...
function validateEnvironment(options = {}) {
  ...
  const rpcResult = validateRpcUrl(
  ...
  if (requireWallets) {
    ...
    const marketingResult = validateAddress(
  ...
  if (requireTokenAddress) {
    const tokenResult = validateAddress(
 ...
 */
async function checkDeploymentBalance(
  provider,
  address,
  minBalanceEther = '0.1',
) {
  try {
    ...
    const minBalanceInWei = ethers.utils.parseEther(minBalanceEther);
 ...
 */
async function validateBalanceOrExit(
  provider,
  address,
  minBalanceEther = '0.1',
) {
  ...
  if (!balanceCheck.sufficient) {
    ...
    console.error(
      colors.red + `   Current: ${balanceCheck.balance} POL` + colors.reset,
    );
    console.error(
      colors.red +
        `   Required: At least ${minBalanceEther} POL for gas fees` +
        colors.reset,
    );
  ...
  if (balanceNum < 0.3) {
    console.log(
      colors.yellow +
        '⚠️  Warning: Balance is low. Recommended to have at least 0.3 POL for safe deployment.' +
        colors.reset,
    );
  }
...
function validateOrExit(options = {}) {
  ...
  if (result.warnings.length > 0) {
    console.log(
      colors.yellow +
        '⚠️  Warnings detected, but continuing...\n' +
        colors.reset,
    );
  }
}

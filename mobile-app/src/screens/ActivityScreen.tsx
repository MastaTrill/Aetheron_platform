...
export const ActivityScreen = () => {
  ...
  const fetchTxs = () => {
    ...
    fetch(
      ...
      .then((data) => {
        ...
        else setError(data.message || 'No transactions found.');
      })
  ...
  useEffect(() => {
    if (!showSwaps) fetchTxs();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [address, showSwaps]);

  if (!address) return <Text style={styles.info}>Connect your wallet to view activity.</Text>;
  if (loading && !showSwaps) return <ActivityIndicator style={{ marginTop: 32 }} />;
  if (error && !showSwaps) return (
        ...
        <FlatList
          ...
          renderItem={({ item }: { item: SwapHistoryItem }) => (
            <View style={styles.item}>
              <Text style={styles.hash}>{item.hash ? item.hash.slice(0, 10) + '...' : 'Local'}</Text>
              <Text style={styles.type}>{item.amount} {item.from.symbol} → {item.minReceived} {item.to.symbol}</Text>
        ...
        <FlatList
          ...
          renderItem={({ item }: { item: Transaction }) => (
            <View style={styles.item}>
              ...
              <Text style={styles.date}>{new Date(Number(item.timeStamp) * 1000).toLocaleString()}</Text>
              <Text style={item.isError === '0' ? styles.success : styles.fail}>
                {item.isError === '0' ? 'Success' : 'Failed'}
              </Text>
            </View>
          )}
          ListEmptyComponent={<Text style={styles.info}>No transactions found for this address.</Text>}
        />
      )}
    </View>
  );

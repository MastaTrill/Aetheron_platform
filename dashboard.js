...
class AetheronDashboard {
	...
	setupEventListeners() {
		...
		if (referralButton) {
			referralButton.addEventListener('click', () => this.generateReferralLink());
		}
	...
	setupAchievementSystem() {
		this.achievements = [
			{ id: 'first-trade', name: 'First Trader', description: 'Complete your first trade', unlocked: false },
			{ id: 'volume-warrior', name: 'Volume Warrior', description: 'Reach $1000 daily volume', unlocked: false },
			{ id: 'community-builder', name: 'Community Builder', description: 'Get 100 social media followers', unlocked: false },
			{ id: 'referral-master', name: 'Referral Master', description: 'Generate 10 referral links', unlocked: false }
		];
	...
	checkTradingMilestones() {
		...
		if (this.tradingVolume >= 1000 && !this.achievements.find(a => a.id === 'volume-warrior').unlocked) {
			this.unlockAchievement('volume-warrior');
		}

		if (this.communityStats.twitterFollowers >= 100 && !this.achievements.find(a => a.id === 'community-builder').unlocked) {
